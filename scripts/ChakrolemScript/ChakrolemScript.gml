...
function ChakrolemState_Charge()
{
	...
	if (xSpeed == 0)
	{
		...
		if (sprite_index != spr_Chakrolem_Charge_Full)
		{
			image_index = 0;
			image_speed = 1;
			Chakrolem_Bomb = instance_create_depth(x, y - 16, depth - 1, obj_ChakrolemBomb);
			Chakrolem_Bomb.ChakrolemBomb_Creator = id;
			sprite_index = spr_Chakrolem_Charge_Full;
			audio_play_sound(sound_ChakrolemBombCharge, 150, false);
		}
		...
		if (image_speed == 0)
		{
			audio_play_sound(sound_ChakrolemBombExplode, 150, false);
